...
router.get('/analiz',async (req,res)=>{
  ...
  try {
    const mf_percent_query = `SELECT 
      SUM(CASE WHEN cinsiyet = 'empty' THEN 1 ELSE 0 END) AS belirsiz_sayısı,
      ...
      COUNT(*) AS toplam,
      (SUM(CASE WHEN cinsiyet = 'empty' THEN 1 ELSE 0 END) / COUNT(*)) * 100 AS belirsiz_oranı,
      ...
      const brans_count_query = `
      SELECT 
        branslar.brans_id, 
        branslar.brans_txt,
        COUNT(aday.kullanici_id) AS aday_sayisi
      FROM 
        branslar
      LEFT JOIN 
        aday ON branslar.brans_id = aday.brans
      GROUP BY 
        branslar.brans_id, branslar.brans_txt;
      `;
      const [brans_count_result] = await db.execute(brans_count_query);
   
    const [mf_result] = await db.execute(mf_percent_query);
    

    console.log(mf_result);  
    console.log(brans_count_result);  

    res.render('analiz', {
      cinsiyet: mf_result[0],
      branslar: brans_count_result
    });
  } catch (error) {
    console.error(error);
    res.status(500).send('Internal Server Error');
